...
BOOL CTrueColorToolBar::LoadTrueColorToolBar( int nImageWidth, int nImageHeight, UINT uToolBar, UINT uToolBarHot /*= 0*/, UINT uToolBarDisabled /*= 0*/ )
{
	if (!SetTrueColorToolBar(TB_SETIMAGELIST, uToolBar, nImageWidth, nImageHeight))
		return FALSE;
	
	if (uToolBarHot) {
		if (!SetTrueColorToolBar(TB_SETHOTIMAGELIST, uToolBarHot, nImageWidth, nImageHeight))
			return FALSE;
	}

	if (uToolBarDisabled) {
		if (!SetTrueColorToolBar(TB_SETDISABLEDIMAGELIST, uToolBarDisabled, nImageWidth, nImageHeight))
			return FALSE;
	}
...
BOOL CTrueColorToolBar::SetTrueColorToolBar( UINT uToolBarType, UINT uToolBar, int nImageWidth, int nImageHeight )
{
	...
	int			nNbBtn	= cSize.cx/nImageWidth;
	RGBTRIPLE*	rgb		= (RGBTRIPLE*)(bmBitmap.bmBits);
	COLORREF	rgbMask	= RGB(rgb[0].rgbtRed, rgb[0].rgbtGreen, rgb[0].rgbtBlue);
	
	if (!cImageList.Create(nImageWidth, nImageHeight, ILC_COLOR24|ILC_MASK, nNbBtn, 0))
		return FALSE;
	...
	// ÉèÖÃ°´Å¥µÄ´óÐ¡£¬ÔÚµ÷ÕûÕû¸ö´°¿Ú´óÐ¡Ê±£¬È·¶¨¹¤¾ßÀ¸µÄ¸ß¶Èµ÷Õû¸÷¸ö×Ó´°¿ÚµÄÎ»ÖÃ
	CSize imageSize(min(nImageWidth, cSize.cx), min(nImageHeight, cSize.cy));
	CSize buttonSize(imageSize.cx + 7, imageSize.cy + 7);
