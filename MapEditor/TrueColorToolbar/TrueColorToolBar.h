...
public:
	...
	// ¼ÓÔØ24Î»Õæ²ÊÉ«¹¤¾ßÀ¸Î»Í¼
	// [²ÎÊý]
	// int nImageWidth µ¥¸ö°´Å¥Î»Í¼¿í
	// int nImageHeight µ¥Î»°´Å¥Î»Í¼¸ß
	// UINT uToolBar Ä¬ÈÏ×´Ì¬µÄÎ»Í¼
	// UINT	uToolBarHot Hot×´Ì¬ÏÂµÄÎ»Í¼£¨Ä¬ÈÏ¿ÉÒÔ²»ËÍ£©
	// UINT uToolDisabled ÎÞÐ§×´Ì¬ÏÂµÄÎ»Í¼£¨Ä¬ÈÏ¿ÉÒÔ²»ËÍ£©
	BOOL LoadTrueColorToolBar(int nImageWidth, int nImageHeight, UINT uToolBar, UINT uToolBarHot = 0, UINT uToolBarDisabled = 0);

	void AddDropDownButton(CWnd* pParent, UINT uButtonID, UINT uMenuID);

private:
	BOOL SetTrueColorToolBar(UINT uToolBarType, UINT uToolBar, int nImageWidth, int nImageHeight);
